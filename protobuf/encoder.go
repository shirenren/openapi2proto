...
func (e *Encoder) writeMessageBlock(name string, src io.Reader) error {
	var indenStr = e.indent
	if name != "message BodyMessage" {
		fmt.Fprintf(e.dst, "\n%s {", name)
	} else {
		indenStr = ""
	}

	n, err := indent(e.dst, src, indenStr)
	if err != nil {
		return errors.Wrap(err, `failed to indent block`)
	}
...
func (e *Encoder) writeRpcFunctionBlock(r *RPC, name string, src io.Reader) error {
	var formatStr = `
...
`
	fmt.Fprintf(e.dst, "\n%s {", name)
	fmt.Fprintf(e.dst, fmt.Sprintf(formatStr, r.method, r.uri))
	fmt.Fprintf(e.dst, "}")
	return nil
}
...
func (e *Encoder) EncodeMessage(v *Message) error {
	...
	for i, field := range v.fields {
		if strings.HasSuffix(v.name, "Request") {
			continue
		}
	...
	if err := e.writeMessageBlock("message "+v.name, &buf); err != nil {
...
func (e *Encoder) EncodeRPC(r *RPC) error {
	...
	if err := e.writeRpcFunctionBlock(r, name, &buf); err != nil {
...
func (e *Encoder) EncodePackage(p *Package) error {
	...
	for _, item := range children {
		...
		case *Message:
			types = append(types, getAllChildren(x)...)
			types = append(types, x)
		default:
			if err := e.EncodeType(item); err != nil {
				return errors.Wrapf(err, `failed to encode %s`, item.Name())
			}
			fmt.Fprintf(e.dst, "\n")
		}
	}
...
func getAllChildren(v Type) (subType []Type) {
	wc, ok := v.(withChildren)
	if !ok {
		return subType
	}

	children := wc.Children()
	for _, item := range children {
		switch item.(type) {
		case *Enum:
			subType = append(subType, getAllChildren(item)...)
			subType = append(subType, item)
		case *Message:
			subType = append(subType, getAllChildren(item)...)
